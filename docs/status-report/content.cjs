      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            ['Database', 'PostgreSQL with 23 tables covering programmes, impact chains, communities, stakeholders, tasks, approvals, targets, spend, reports, uploads and media coverage. Schema changes are versioned migrations, applied deliberately as part of a release rather than as a side effect of one.'],
            ['Application interface', '48 API endpoints, all authenticated. Written as hosting-agnostic handlers called by a single adapter, so managed hosting runs the same application as a Seplat-hosted install rather than a second implementation of it.'],
            ...
            ['Alerts on coverage that matters', 'A watchlist of named rules. A newly collected story matching one is posted to Microsoft Teams or Slack when it is found, rather than waiting for somebody to open the screen. Rules are deliberately narrow — everything else still arrives in the queue — because alerting on all coverage teaches people to ignore the alerts. A webhook that stops working reports itself on the screen instead of failing silently.'],
            ['A queue that stays current', 'The review screen refreshes itself while it is open and marks what has arrived since it was opened, so a story collected during a meeting is visible by the end of it rather than on the next reload.'],
